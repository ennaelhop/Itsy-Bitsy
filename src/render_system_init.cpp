...
bool RenderSystem::init(int width, int height, GLFWwindow* window_arg)
{
	...
	float scale = 1200.f/virtual_W; // used only for the next matrix
	screen_to_world_matrix = mat3({scale,0,0}, { 0, scale,0 }, {-(scale * vp_x),-(scale * vp_y),1.f});
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	////////////////////////////////////////////////////////
	// Initialise a point... (Is this necessary? If for no other reasonthan having similar code as other shaders)
	std::vector<vec3> point_vertex(1);
	point_vertex[0] = { 0.f, 0.f, 0.f }; //a point is really just vertex centered at object coord origin.
	glBindBuffer(GL_ARRAY_BUFFER, vertex_buffers[(uint)GEOMETRY_BUFFER_ID::POINT_PARTICLE]); //we also only need to bind an array buffer, no indices.
	glBufferData(GL_ARRAY_BUFFER,
		sizeof(vec3), point_vertex.data(), GL_STATIC_DRAW);
	gl_has_errors();
}
	...
	const std::string& vs_path, const std::string& fs_path, const std::string& gs_path, GLuint& out_program)
{
	...
	GLuint geometry;
	if (gs_is.good()) {//if we have a geometry shader, load and compile and link it too
		std::stringstream gs_ss;
		gs_ss << gs_is.rdbuf();
	...
	if (gs_is.good()) { // if we have a geometry shade, delete it too
